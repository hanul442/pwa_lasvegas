...
export function startBlackjack(stake) {
  ...
  const round={id:id('bj'),game:'Blackjack',stake,deck,player,dealer,status:'PLAYER',doubled:false,createdAt:new Date().toISOString()};
  const p=bjValue(player),d=bjValue(dealer);
  if(p.total===21 || d.total===21) round.status='READY_TO_SETTLE';
  return round;
}
export function blackjackPublic(round, reveal=false){return {id:round.id,game:round.game,stake:round.stake,status:round.status,player:round.player.map(cardText),playerValue:bjValue(round.player).total,dealer:[cardText(round.dealer[0]),reveal?cardText(round.dealer[1]):'HIDDEN'],dealerValue:reveal?bjValue(round.dealer).total:null,doubled:round.doubled};}
...
function baccaratCardValue(c){ if(c.r==='A') return 1; if(['10','J','Q','K'].includes(c.r)) return 0; return Number(c.r); }
function baccaratScore(cards){return cards.reduce((s,c)=>s+baccaratCardValue(c),0)%10;}
export function playBaccarat(stake,betType){
  if(!['PLAYER','BANKER','TIE'].includes(betType)) throw new Error('INVALID_BET_TYPE');
  const deck=freshDeck(8); const player=[deck.pop(),deck.pop()]; const banker=[deck.pop(),deck.pop()];
  let ps=baccaratScore(player),bs=baccaratScore(banker); let pThird=null;
  if(ps<8&&bs<8){
    if(ps<=5){pThird=deck.pop();player.push(pThird);ps=baccaratScore(player);}
    if(!pThird){if(bs<=5) banker.push(deck.pop());}
    else {
      const pt=baccaratCardValue(pThird);
      if(bs<=2 || (bs===3&&pt!==8) || (bs===4&&pt>=2&&pt<=7) || (bs===5&&pt>=4&&pt<=7) || (bs===6&&pt>=6&&pt<=7)) banker.push(deck.pop());
    }
    bs=baccaratScore(banker);
  }
  const winner=ps>bs?'PLAYER':bs>ps?'BANKER':'TIE';
  let netPnl;
  if(winner==='TIE' && betType!=='TIE') netPnl=0;
  else if(winner!==betType) netPnl=-stake;
  else if(betType==='PLAYER') netPnl=stake;
  else if(betType==='BANKER') netPnl=Math.floor(stake*0.95);
  else netPnl=stake*8;
  return {id:id('bac'),game:'Baccarat',stake,betType,winner,player:player.map(cardText),banker:banker.map(cardText),playerScore:ps,bankerScore:bs,netPnl};
}
