...
const TABLES=[
  {id:'obsidian-blackjack',gameId:'blackjack',game:'Blackjack',name:'Obsidian Blackjack',tag:'MULTI-SPOT · SPLIT · DOUBLE'},
  {id:'rouge-baccarat',gameId:'baccarat',game:'Baccarat',name:'Rouge Baccarat',tag:'NATURAL · THIRD-CARD AUDIT'},
  {id:'zero-chamber',gameId:'roulette',game:'Roulette',name:'Zero Chamber',tag:'EUROPEAN · SINGLE ZERO'},
  {id:'dragon-dice',gameId:'sicbo',game:'Sic Bo',name:'Dragon Dice',tag:'TRIPLE · TOTAL · BIG/SMALL'}
];
...
export function vaultEntryIntent(gameId,unlocked){const table=TABLES.find(t=>t.gameId===gameId);if(!table||!unlocked)return null;return{floor:'vault',gameId:table.gameId,tableId:table.id};}
...
function enterVaultGame(gameId){const button=document.querySelector(`[data-vault-game="${gameId}"]`);if(!button||button.disabled)return;const intent=vaultEntryIntent(gameId,true);if(!intent)return;localStorage.setItem('sv-floor',intent.floor);sessionStorage.setItem('sv-vault-game',intent.gameId);location.href='/?page=home';}
function resumeVaultGame(){const gameId=sessionStorage.getItem('sv-vault-game');if(!gameId)return false;const target=document.querySelector(`[data-game="${gameId}"]`);if(!target)return false;sessionStorage.removeItem('sv-vault-game');target.click();return true;}
if(typeof document!=='undefined'){document.addEventListener('click',event=>{const button=event.target.closest?.('[data-vault-game]');if(button)enterVaultGame(button.dataset.vaultGame);});let queued=false;const schedule=()=>{if(queued)return;queued=true;requestAnimationFrame(async()=>{queued=false;if(resumeVaultGame())return;await decorate();});};addEventListener('DOMContentLoaded',schedule,{once:true});new MutationObserver(schedule).observe(document.documentElement,{childList:true,subtree:true});}
