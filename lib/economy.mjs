...
const GAME_SETTLEMENT_TYPES=new Set(['GAME_WIN','GAME_LOSS','GAME_PUSH']);

function today() { return new Date().toISOString().slice(0,10); }

export function totalBalance(user) { return user.account.available + user.account.locked; }

function assertRoundUnsettled(state,user,roundId){
  const duplicate=Array.isArray(state?.ledger)&&state.ledger.some(tx=>
    tx?.userId===user.id&&
    tx.referenceType==='GAME_ROUND'&&
    tx.referenceId===roundId&&
    GAME_SETTLEMENT_TYPES.has(tx.type)
  );
  if(duplicate)throw new Error('ROUND_ALREADY_SETTLED');
}
function assertStake(stake){
  if(!Number.isSafeInteger(stake)||stake<=0)throw new Error('INVALID_STAKE');
}
function assertSettlement(stake,netPnl){
  assertStake(stake);
  if(!Number.isSafeInteger(netPnl)||netPnl < -stake)throw new Error('INVALID_SETTLEMENT');
  if(!Number.isSafeInteger(stake+netPnl))throw new Error('INVALID_SETTLEMENT');
}
function assertLedgerDeltas(amount,lockedDelta,rawPnlDelta,qualifiedPnlDelta){
  if(![amount,lockedDelta,rawPnlDelta,qualifiedPnlDelta].every(Number.isSafeInteger))throw new Error('INVALID_LEDGER_DELTA');
}
...
export function addLedger(state, user, {type, amount, lockedDelta=0, rawPnlDelta=0, qualifiedPnlDelta=0, referenceType='SYSTEM', referenceId=null, metadata={}}) {
  assertLedgerDeltas(amount,lockedDelta,rawPnlDelta,qualifiedPnlDelta);
  const before = user.account.available;
  const lockedBefore = user.account.locked;
  const after = before + amount;
  const lockedAfter = lockedBefore + lockedDelta;
  if (!Number.isSafeInteger(after) || !Number.isSafeInteger(lockedAfter)) throw new Error('INVALID_LEDGER_DELTA');
...
export function settleLockedHouseGame(state,user,{game,stake,netPnl,roundId,details={}}) {
  assertSettlement(stake,netPnl);
  assertRoundUnsettled(state,user,roundId);
  ...
  if(!Number.isSafeInteger(user.account.totalWagered))throw new Error('INVALID_SETTLEMENT');
